...
const els = {
  ...
  teamAssignFile: document.getElementById('team-assign-file'),
  applyTeamAssignment: document.getElementById('apply-team-assignment'),
  ...
  openScoreboard: document.getElementById('open-scoreboard'),
  exportResults: document.getElementById('export-results'),
  showFinalResults: document.getElementById('show-final-results'),
  scoreboardOverlay: document.getElementById('scoreboard-overlay'),
  closeScoreboard: document.getElementById('close-scoreboard'),
  scoreboardBody: document.getElementById('scoreboard-body'),
  scoreboardSubtitle: document.getElementById('scoreboard-subtitle'),
  ...
  toggleAnswerTop: document.getElementById('toggle-answer-top'),
  ...
  jeopardyCloseSubmission: document.getElementById('jeopardy-close-submission'),
  jeopardyToggleAnswer: document.getElementById('jeopardy-toggle-answer'),
  jeopardyOpenScoreboard: document.getElementById('jeopardy-open-scoreboard'),
  jeopardyExportResults: document.getElementById('jeopardy-export-results'),
  jeopardyShowFinalResults: document.getElementById('jeopardy-show-final-results'),
...
const state = {
  ...
  formEditMode: true,
  scoreboardShownAt: 0,
  jeopardyReturnMode: 'board',
  teamAssignmentEditingId: '',
  autoClosingRoundId: '',
  historyReady: false
};
...
function gameHasStarted() {
  return ['waiting', 'countdown', 'live', 'closed', 'reveal', 'ended'].includes(state.gameState.phase);
}

function playedProblemIds() {
  return Array.isArray(state.gameState.playedProblemIds) ? state.gameState.playedProblemIds : [];
}

function isActiveProblemPlayPhase() {
  return Boolean(state.gameState.sessionId) && ['countdown', 'live', 'closed', 'reveal'].includes(state.gameState.phase);
}

function isProblemPlayedInCurrentActiveRound(problemId) {
  return isActiveProblemPlayPhase() && playedProblemIds().includes(problemId);
}

async function maybeGradeRoundSubmissions() {
  if (!isOperator() || state.gradingInFlight) return;
  const roundId = state.gameState.currentRoundId;
  const problem = state.problems.find((p) => p.id === state.gameState.currentProblemId) || null;
  if (!roundId || !problem || !state.firebase?.fsMod || !state.db) return;
  const roundSubs = state.submissions.filter((s) => s.sessionId === state.gameState.sessionId && s.roundId === roundId && s.problemId === problem.id);
  if (!roundSubs.length) return;
  const { fsMod } = requireFirebaseReady();
  state.gradingInFlight = true;
  try {
    for (const submission of roundSubs) {
      const graded = autoGradeSubmission(problem, submission.answers || []);
      const gradeKey = JSON.stringify({ a: submission.answers || [], p: problem.answer || '', slots: problem.answerSlotCount || 1, score: problem.score || 0 });
      if (submission.gradeKey === gradeKey && Array.isArray(submission.correctness) && submission.correctness.length) continue;
      await fsMod.setDoc(fsMod.doc(state.db, 'gameSubmissions', submission.id), {
    ...
    state.gradingInFlight = false;
  }
}

function currentSessionSubmissions() {
  if (!state.gameState.sessionId) return [];
  return state.submissions.filter((submission) => submission.sessionId === state.gameState.sessionId);
}
...
function activeGameRosterIds() {
  const roster = Array.isArray(state.gameState.rosterIds) ? state.gameState.rosterIds : [];
  if (roster.length) return roster;
  const finalRoster = Array.isArray(state.gameState.finalRosterIds) ? state.gameState.finalRosterIds : [];
  return finalRoster;
}

function teamMembersText(teamId, submission = null) {
  const rosterIds = activeGameRosterIds();
  const members = state.participants
    .filter((participant) => participant.teamId === teamId)
    .filter((participant) => !rosterIds.length || rosterIds.includes(participant.id))
    .map((participant) => participant.displayName || participant.email || '이름 없음');
  if (members.length) return members.join(', ');
  if (submission?.submittedByName) return submission.submittedByName;
  return '팀원이 아직 표시되지 않습니다.';
}

function currentGameParticipants() {
  const rosterIds = activeGameRosterIds();
  if (rosterIds.length) return state.participants.filter((participant) => rosterIds.includes(participant.id)).slice().sort((a, b) => participantSortKey(a).localeCompare(participantSortKey(b), 'ko') || String(a.id || '').localeCompare(String(b.id || '')));
  const submittedIds = new Set(currentSessionSubmissions().map((submission) => submission.submittedBy).filter(Boolean));
  if (submittedIds.size) return state.participants.filter((participant) => submittedIds.has(participant.id)).slice().sort((a, b) => participantSortKey(a).localeCompare(participantSortKey(b), 'ko') || String(a.id || '').localeCompare(String(b.id || '')));
  return [];
}

function activeRoundTeamIds() {
  const rosterIds = Array.isArray(state.gameState.rosterIds) ? state.gameState.rosterIds : [];
  const source = rosterIds.length ? state.participants.filter((participant) => rosterIds.includes(participant.id)) : visibleParticipants();
  return [...new Set(source.map((participant) => participant.teamId).filter(Boolean))];
}

function submittedRoundTeamIds() {
  return [...new Set(roundSubmissionsSorted().map((submission) => submission.teamId).filter(Boolean))];
}

function areAllActiveTeamsSubmitted() {
  const activeTeams = activeRoundTeamIds();
  if (!activeTeams.length || !state.gameState.currentRoundId || !state.gameState.currentProblemId) return false;
  const submitted = new Set(submittedRoundTeamIds());
  return activeTeams.every((teamId) => submitted.has(teamId));
}

async function maybeAutoCloseWhenAllTeamsSubmitted() {
  if (!isOperator()) return;
  if (state.gameState.phase !== 'live' || state.gameState.submissionClosed) return;
  if (!areAllActiveTeamsSubmitted()) return;
  if (state.autoClosingRoundId === state.gameState.currentRoundId) return;
  state.autoClosingRoundId = state.gameState.currentRoundId;
  try {
    await finishRoundAndClose();
    toast('모든 팀이 제출하여 자동으로 마감되었습니다.', 'success');
  } catch (error) {
    console.error(error);
    state.autoClosingRoundId = '';
  }
}

function roundSubmissionsSorted() {
  return currentSessionSubmissions()
    .filter((submission) => submission.roundId === state.gameState.currentRoundId && submission.problemId === state.gameState.currentProblemId)
    .slice()
    .sort((a, b) => Number(a.elapsedMs || 0) - Number(b.elapsedMs || 0) || String(a.submittedAt || '').localeCompare(String(b.submittedAt || '')));
}

function renderLiveSubmissionPanel() {
  const submissions = roundSubmissionsSorted();
  const allDone = areAllActiveTeamsSubmitted();
  if (!submissions.length) {
    return '<div class="live-submission-panel"><h4>실시간 제출 현황</h4><div class="empty-box compact-empty">아직 제출한 팀이 없습니다.</div></div>';
  }
  return `<div class="live-submission-panel"><h4>실시간 제출 현황</h4>${allDone ? '<div class="all-submitted-banner">모든 팀이 다 제출했습니다!</div>' : ''}<div class="live-submission-list">${submissions.map((submission, index) => {
    const team = state.teams.find((item) => item.id === submission.teamId);
    return `<div class="live-submission-row"><div class="live-submission-rank">${index + 1}</div><div><strong>${escapeHtml(team?.name || '팀 미배정')}</strong><small>${escapeHtml(teamMembersText(submission.teamId, submission))}</small></div><span>${escapeHtml(formatElapsedMs(submission.elapsedMs || 0))}</span></div>`;
  }).join('')}</div></div>`;
}

function computeRoundTimeDeltaMap() {
  const deltaMap = new Map(state.teams.map((team) => [team.id, 0]));
  currentSessionSubmissions().filter((s) => s.roundId === state.gameState.currentRoundId).forEach((submission) => {
    deltaMap.set(submission.teamId, (deltaMap.get(submission.teamId) || 0) + Number(submission.elapsedMs || 0));
  });
  return deltaMap;
}

function computeTeamTotals() {
  const totals = new Map(state.teams.map((team) => [team.id, { total: 0, timeMs: 0 }]));
  currentSessionSubmissions().forEach((submission) => {
    const current = totals.get(submission.teamId) || { total: 0, timeMs: 0 };
    current.total += Number(submission.scoreAwarded || 0);
    current.timeMs += Number(submission.elapsedMs || 0);
    totals.set(submission.teamId, current);
  });
  return state.teams.map((team) => ({ teamId: team.id, teamName: team.name, total: totals.get(team.id)?.total || 0, timeMs: totals.get(team.id)?.timeMs || 0 })).sort((a, b) => b.total - a.total || a.timeMs - b.timeMs || a.teamName.localeCompare(b.teamName, 'ko'));
}
...
function participantSortKey(participant) {
  return String(participant.createdAt || participant.joinedAt || participant.displayName || participant.email || participant.id || '');
}
...
function visibleParticipants() {
  ...
  }).slice().sort((a, b) => participantSortKey(a).localeCompare(participantSortKey(b), 'ko') || String(a.id || '').localeCompare(String(b.id || '')));
}
...
function bindEvents() {
  ...
  els.backToAuthorBrowser.addEventListener('click', () => { state.authorMode = 'browser'; state.selectedProblemId = ''; state.loadedProblemId = ''; renderAuthorView(); pushCurrentRoute(); });
  ...
  els.applyTeamAssignment?.addEventListener('click', applyTeamAssignmentFromExcel);
  ...
  els.toggleAnswerTop?.addEventListener('click', toggleAnswerVisible);
  els.toggleAnswer.addEventListener('click', toggleAnswerVisible);
  els.openScoreboard?.addEventListener('click', () => openScoreboardOverlay(false));
  els.exportResults?.addEventListener('click', exportResultsWorkbook);
  els.showFinalResults?.addEventListener('click', () => showFinalResults(false));
  ...
  els.jeopardyCloseSubmission?.addEventListener('click', closeSubmissions);
  els.jeopardyToggleAnswer?.addEventListener('click', toggleAnswerVisible);
  els.jeopardyOpenScoreboard?.addEventListener('click', () => openScoreboardOverlay(true));
  els.jeopardyExportResults?.addEventListener('click', exportResultsWorkbook);
  els.jeopardyShowFinalResults?.addEventListener('click', () => showFinalResults(true));
  els.jeopardyBack.addEventListener('click', () => { state.jeopardyMode = ['scoreboard', 'finale'].includes(state.jeopardyMode) ? state.jeopardyReturnMode : 'board'; renderJeopardyOverlay(); });
  ...
  els.closeScoreboard?.addEventListener('click', closeScoreboardOverlay);
  els.scoreboardOverlay?.addEventListener('click', (event) => { if (event.target === els.scoreboardOverlay) closeScoreboardOverlay(); });
...
function subscribeAll() {
  ...
  if (problemsRef) subscribe(problemsRef, (snap) => {
    ...
    if (isOperator()) maybeGradeRoundSubmissions();
      if (!state.selectedReviewProblemId && reviewableProblems()[0]) state.selectedReviewProblemId = reviewableProblems()[0].id;
    if (!state.selectedGameProblemId && approvedProblems()[0]) state.selectedGameProblemId = approvedProblems()[0].id;
    renderOverview();
    renderAuthorView();
  ...
  subscribe(fsMod.collection(state.db, 'gameSubmissions'), (snap) => {
    ...
    if (isOperator()) { maybeGradeRoundSubmissions(); maybeAutoCloseWhenAllTeamsSubmitted(); }
    renderPlayView();
    renderGameStage();
...
function renderGameAdmin() {
  ...
  if (!state.teamAssignmentEditingId) {
...
function renderJeopardyOverlay() {
  ...
  const isFinaleMode = state.jeopardyMode === 'finale';
  els.jeopardyOverlay.classList.toggle('jeopardy-finale-mode', isFinaleMode);
  els.jeopardyBody.classList.toggle('jeopardy-finale-body', isFinaleMode);
  els.jeopardyBack.classList.toggle('hidden', !['problem', 'scoreboard', 'finale'].includes(state.jeopardyMode));
  ...
  els.jeopardyCloseSubmission?.classList.toggle('hidden', state.jeopardyMode !== 'problem');
  els.jeopardyToggleAnswer?.classList.toggle('hidden', state.jeopardyMode !== 'problem');
  if (state.jeopardyMode === 'finale') {
    renderJeopardyFinale();
  } else if (state.jeopardyMode === 'problem') {
    delete els.jeopardyBody.dataset.finaleKey;
    renderJeopardyProblem();
  } else if (state.jeopardyMode === 'scoreboard') {
    delete els.jeopardyBody.dataset.finaleKey;
    renderJeopardyScoreboard();
  } else {
    delete els.jeopardyBody.dataset.finaleKey;
    renderJeopardyBoard();
  }
...
function renderJeopardyBoard() {
  ...
  els.jeopardyBody.innerHTML = list.length ? `<div class="jeopardy-grid">${list.map((problem) => `
    <button class="jeopardy-card ${problem.id === state.selectedGameProblemId ? 'selected' : ''} ${isProblemPlayedInCurrentActiveRound(problem.id) ? 'solved' : ''}" type="button" data-jeopardy-problem="${escapeAttr(problem.id)}">
      ...
      <span class="jeopardy-meta">${escapeHtml(problem.track || '분야')} · ${escapeHtml(String(problem.difficulty || '중'))} · ${escapeHtml(String(problem.score || 0))}점${isProblemPlayedInCurrentActiveRound(problem.id) ? ' · 완료' : ''}</span>
...
function renderJeopardyProblem() {
  ...
  els.jeopardyBody.innerHTML = `<div class="jeopardy-problem-wrap">${presenterHeaderMarkup(problem)}${buildProblemMarkup(problem, 'desktop', isOperator() && state.gameState.answerVisible, { presenter: true })}${countdownOverlayMarkup(false)}</div>`;
  scheduleTypeset(els.jeopardyBody);
}


function presenterHeaderMarkup(problem) {
  return `<div class="presenter-problem-header"><div><span class="presenter-kicker">${escapeHtml(problem.detail || '수강과목')} · ${escapeHtml(problem.track || '분야')}</span><h2>${escapeHtml(problem.title || '제목 없는 문제')}</h2></div><div class="presenter-core-chips"><span>${escapeHtml(String(problem.score || 0))}점</span><span>${escapeHtml(problem.difficulty || '중')}</span><span>${escapeHtml(problem.solveTimeMinutes ? `${problem.solveTimeMinutes}분` : '풀이 시간')}</span></div></div>`;
}

function renderGameStage() {
  if (state.gameState.phase === 'ended') {
    els.currentRoundLabel.textContent = GAME_PHASE_LABELS[state.gameState.phase] || '게임이 종료되었습니다.';
    if (shouldRenderFinale()) {
      renderFinalCeremonyTo(els.presenterShell, true);
    } else {
      delete els.presenterShell.dataset.finaleKey;
      els.presenterShell.innerHTML = '<div class="empty-box game-ended-box">게임이 종료되었습니다.</div>';
    }
  ...
  els.presenterShell.innerHTML = `<div class="present-problem-wrap">${presenterHeaderMarkup(problem)}${buildProblemMarkup(problem, mode, isOperator() && state.gameState.answerVisible, { presenter: true })}${countdownOverlayMarkup(false)}</div>`;
  scheduleTypeset(els.presenterShell);
  renderRoundResults();
  renderJeopardyOverlay();
}

function finaleElapsedMs() {
  return Math.max(0, Date.now() - Number(state.gameState.finaleStartedAt || 0));
}

function finaleDelayStyle(delayMs) {
  const remaining = delayMs - finaleElapsedMs();
  return `--delay:${delayMs}ms;--play-delay:${remaining}ms;`;
}

function finalPodiumCard(rank, item, delay, label) {
  const style = finaleDelayStyle(delay);
  if (!item) return `<div class="finale-reveal-card finale-reveal-empty finale-rank-${rank}" style="${style}"><span>${rank}위</span></div>`;
  return `<div class="finale-reveal-card finale-rank-${rank}" style="${style}">
    <div class="spotlight-beam"></div>
    <div class="finale-curtain">${rank}위</div>
    <div class="finale-stage-label">${escapeHtml(label)}</div>
    <div class="finale-stage-rank">${rank === 1 ? `<img class="scoreboard-crown" src="./assets/crown.png" alt="1위" />` : ''}<strong>${rank}위</strong></div>
    <div class="finale-stage-team">${escapeHtml(item.teamName)}</div>
    <div class="finale-stage-score">${escapeHtml(String(item.total))}점</div>
    <div class="finale-stage-time">총 제출 시간 ${escapeHtml(formatElapsedMs(item.timeMs))}</div>
  </div>`;
}

function finalCeremonyMarkup(fullscreen = false) {
  const totals = computeTeamTotals();
  const elapsed = finaleElapsedMs();
  const listDelay = totals[0] ? 11100 : 7200;
  const countdown = `
    <div class="finale-countdown finale-countdown-big">
      <span class="countdown-three" style="animation-delay:${100 - elapsed}ms">3</span>
      <span class="countdown-two" style="animation-delay:${1100 - elapsed}ms">2</span>
      <span class="countdown-one" style="animation-delay:${2100 - elapsed}ms">1</span>
    </div>`;
  const stageMarkup = totals.length ? `<div class="finale-reveal-grid">
      <div class="finale-slot finale-slot-left">${finalPodiumCard(2, totals[1], 7000, '2등 발표')}</div>
      <div class="finale-slot finale-slot-center">${finalPodiumCard(1, totals[0], 10600, '1등 발표')}</div>
      <div class="finale-slot finale-slot-right">${finalPodiumCard(3, totals[2], 3400, '3등 발표')}</div>
    </div>` : '<div class="empty-box finale-empty" style="animation-delay:3200ms">표시할 최종 순위가 없습니다.</div>';
  const fullList = totals.length ? `<div class="finale-list finale-list-delayed" style="--delay:${listDelay}ms;animation-delay:${listDelay - elapsed}ms">${totals.map((item, index) => `<div class="finale-list-row" style="animation-delay:${listDelay + 90 * index - elapsed}ms"><span>${index + 1}위</span><strong>${escapeHtml(item.teamName)}</strong><span>${escapeHtml(String(item.total))}점</span><small>${escapeHtml(formatElapsedMs(item.timeMs))}</small></div>`).join('')}</div>` : '';
  return `<div class="finale-wrap finale-wrap-v56 ${fullscreen ? 'finale-wrap-full' : ''}" data-finale-start="${escapeAttr(String(state.gameState.finaleStartedAt || ''))}"><div class="finale-title">최종 결과 발표</div>${countdown}${stageMarkup}${fullList}</div>`;
}

function hasActiveFinaleData() {
  const startedAt = Number(state.gameState.finaleStartedAt || 0);
  const freshEnough = startedAt > 0 && (Date.now() - startedAt) < 10 * 60 * 1000;
  const finaleSessionId = String(state.gameState.finaleSessionId || '');
  const currentSessionId = String(state.gameState.sessionId || '');
  const playedCount = Number(state.gameState.finalPlayedProblemCount || 0);
  const hasCurrentSessionSubmissions = currentSessionSubmissions().length > 0;
  return state.gameState.phase === 'ended'
    && freshEnough
    && finaleSessionId
    && currentSessionId
    && finaleSessionId === currentSessionId
    && playedCount > 0
    && hasCurrentSessionSubmissions
    && computeTeamTotals().some((team) => Number(team.total || 0) > 0 || Number(team.timeMs || 0) > 0);
}

function shouldRenderFinale() {
  return hasActiveFinaleData();
}

function shouldRenderPlayerFinale() {
  const participant = currentParticipant();
  const finalRosterIds = Array.isArray(state.gameState.finalRosterIds) ? state.gameState.finalRosterIds : [];
  return hasActiveFinaleData() && Boolean(participant) && finalRosterIds.includes(participant.id);
}

function finalCeremonyKey(fullscreen = false) {
  return [
    state.gameState.sessionId || '',
    state.gameState.finaleSessionId || '',
    state.gameState.finaleStartedAt || '',
    fullscreen ? 'full' : 'normal'
  ].join('::');
}

function renderFinalCeremonyTo(root, fullscreen = false) {
  const key = finalCeremonyKey(fullscreen);
  if (root.dataset.finaleKey === key && root.querySelector('.finale-wrap')) return;
  root.dataset.finaleKey = key;
  root.innerHTML = finalCeremonyMarkup(fullscreen);
}

function renderJeopardyScoreboard() {
  els.jeopardyTitle.textContent = '실시간 순위표';
  els.jeopardyBody.innerHTML = `<div class="jeopardy-scoreboard-wrap">${scoreboardRowsMarkup(true)}</div>`;
}

function renderJeopardyFinale() {
  els.jeopardyOverlay.classList.add('jeopardy-finale-mode');
  els.jeopardyBody.classList.add('jeopardy-finale-body');
  if (!shouldRenderFinale()) {
    els.jeopardyTitle.textContent = '게임이 종료되었습니다.';
    delete els.jeopardyBody.dataset.finaleKey;
    els.jeopardyBody.innerHTML = '<div class="empty-box">게임이 종료되었습니다.</div>';
    return;
  }
  els.jeopardyTitle.textContent = '최종 결과 발표';
  renderFinalCeremonyTo(els.jeopardyBody, true);
}

function renderPlayView() {
  els.playerStatusBadge.textContent = GAME_PHASE_LABELS[state.gameState.phase] || '대기 중입니다.';
  if (state.gameState.phase === 'ended') {
    if (shouldRenderPlayerFinale()) {
      renderFinalCeremonyTo(els.playShell, true);
    } else {
      delete els.playShell.dataset.finaleKey;
      els.playShell.innerHTML = '<div class="empty-box">게임이 종료되었습니다.</div>';
    }
...
function renderPlayerJoinPanel() {
  ...
  if (!participant) {
    if (gameHasStarted()) {
      els.playerJoinPanel.innerHTML = '<div class="empty-box">게임이 이미 시작되어 새로운 참가자는 지금 입장할 수 없습니다.</div>';
      return;
    }
    els.playerJoinPanel.innerHTML = `
  ...
  const readyLocked = gameHasStarted();
  els.playerJoinPanel.innerHTML = `
    <div class="section-head compact"><h3>참가 상태</h3><span class="note">${readyLocked ? '게임이 시작되어 준비 상태를 변경할 수 없습니다.' : (participant.ready ? '준비 완료입니다.' : '준비 전입니다.')}</span></div>
    <div class="summary-box">${escapeHtml(participant.displayName)} · ${escapeHtml(teamName(participant.teamId) || '팀 미배정입니다.')}</div>
    <div class="action-row">
      <button id="toggle-ready-button" class="btn ${participant.ready ? 'ghost' : 'primary'}" type="button" ${readyLocked ? 'disabled' : ''}>${participant.ready ? '준비 해제' : '준비 완료'}</button>
...
function decodeStoredAnswerText(text) {
...
function resultAnswerHtml(text) {
  const value = decodeStoredAnswerText(text);
  return value && value.trim() ? formatTextHtml(value) : '<p>없음</p>';
}
...
function renderPlayerResultPanel() {
  ...
  const labels = answerLabels(problem);
  const expected = Array.isArray(submission.expected) && submission.expected.length ? submission.expected : splitExpectedAnswers(problem);
  ...
  els.playerResultPanel.innerHTML = `
    ...
    <div class="card-list compact-list">
      ${labels.map((label, i) => `
        <div class="item-card score-card score-detail-card">
          <div>
            <strong>${escapeHtml(label)}</strong>
            <div class="answer-block">
              <div>
                <div class="score-label">제출</div>
                <div class="result-answer-rich math-region">${resultAnswerHtml(answers[i])}</div>
              </div>
              <div>
                <div class="score-label">정답</div>
                <div class="result-answer-rich math-region">${resultAnswerHtml(expected[i])}</div>
              </div>
            </div>
          </div>
          <span class="${submission.correctness?.[i] ? 'result-good' : 'result-bad'}">${submission.correctness?.[i] ? '정답' : '오답'}</span>
        </div>`).join('')}
    </div>
  `;
  scheduleTypeset(els.playerResultPanel);
  requestAnimationFrame(() => scheduleTypeset(els.playerResultPanel));
}
...
async function onLogout() {
  ...
  try {
    ...
    await initialLoad();
    subscribeAll();
    state.authorMode = 'browser';
    state.selectedProblemId = '';
    state.loadedProblemId = '';
    state.jeopardyMode = 'board';
    state.jeopardyReturnMode = 'board';
    els.jeopardyOverlay?.classList.add('hidden');
    closeScoreboardOverlay();
    switchView('overview-view', { replaceHistory: true });
    refreshAll();
...
async function openLobby() {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  await setGameState({ phase: 'lobby', sessionId: '', currentRoundId: '', currentProblemId: '', publicProblem: null, answerVisible: false, submissionClosed: false, rosterIds: [], playedProblemIds: [], finalRosterIds: [], finaleStartedAt: null, finaleSessionId: '', finalPlayedProblemCount: 0, readyPrompt: null });
...
async function startGame() {
  ...
  await setGameState({ phase: 'waiting', sessionId: uid('session'), rosterIds, currentRoundId: '', currentProblemId: '', publicProblem: null, answerVisible: false, submissionClosed: false, playedProblemIds: [], finalRosterIds: [], finaleStartedAt: null, finaleSessionId: '', finalPlayedProblemCount: 0, readyPrompt: null });
  toast('게임이 시작되었습니다.', 'success');
}

async function endGame() {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  await setGameState({ phase: 'ended', currentRoundId: '', currentProblemId: '', publicProblem: null, answerVisible: true, submissionClosed: true, finalRosterIds: Array.isArray(state.gameState.rosterIds) ? state.gameState.rosterIds : [], rosterIds: [], playedProblemIds: playedProblemIds(), finaleStartedAt: null, finaleSessionId: '', finalPlayedProblemCount: playedProblemIds().length, roundEndsAt: null });
  if (els.jeopardyOverlay && !els.jeopardyOverlay.classList.contains('hidden')) state.jeopardyMode = 'board';
  refreshAll();
  toast('게임이 종료되었습니다. 최종 연출은 최종 등수 보기를 눌러 표시합니다.', 'success');
}

async function showFinalResults(fromJeopardy = false) {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  if (!state.gameState.sessionId) return toast('최종 결과를 표시할 게임 세션이 없습니다.', 'error');
  if (!playedProblemIds().length || !currentSessionSubmissions().length) return toast('최종 결과를 표시하려면 최소 1개 문제와 제출 기록이 필요합니다.', 'error');
  await setGameState({
    phase: 'ended',
    currentRoundId: '',
    currentProblemId: '',
    ...
    finalRosterIds: Array.isArray(state.gameState.finalRosterIds) && state.gameState.finalRosterIds.length ? state.gameState.finalRosterIds : (Array.isArray(state.gameState.rosterIds) ? state.gameState.rosterIds : []),
    rosterIds: [],
    playedProblemIds: playedProblemIds(),
    finaleStartedAt: Date.now(),
    finaleSessionId: state.gameState.sessionId || '',
    finalPlayedProblemCount: playedProblemIds().length,
    roundEndsAt: null
  });
  if (fromJeopardy && els.jeopardyOverlay && !els.jeopardyOverlay.classList.contains('hidden')) state.jeopardyMode = 'finale';
  refreshAll();
  toast('최종 등수 발표를 시작합니다.', 'success');
}
...
async function openSelectedProblemForGame() {
  ...
  await setGameState({ phase: 'countdown', currentRoundId: uid('round'), currentProblemId: problem.id, publicProblem: publicProblemPayload(problem, false), answerVisible: false, submissionClosed: false, playedProblemIds: [...new Set([...playedProblemIds(), problem.id])], roundStartsAt: Date.now() + 4000, roundEndsAt: null });
  toast('선택한 문제가 송출되었습니다. 참가자 화면에는 카운트다운 후 표시됩니다.', 'success');
}

async function closeSubmissions() {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  await maybeGradeRoundSubmissions();
  await finishRoundAndClose();
  refreshAll();
...
async function toggleAnswerVisible() {
  ...
  if (next) await maybeGradeRoundSubmissions();
  ...
  await setGameState({ answerVisible: next, publicProblem: publicProblemPayload(currentLiveProblem(), next), phase: nextPhase });
  refreshAll();
...
async function joinGame() {
  ...
  if (gameHasStarted()) return toast('게임이 이미 시작되어 지금은 새로 참가할 수 없습니다.', 'error');
...
async function toggleReady() {
  ...
  if (gameHasStarted()) return toast('게임이 시작된 뒤에는 준비 상태를 변경할 수 없습니다.', 'error');
  const { fsMod } = requireFirebaseReady();
...
async function submitTeamAnswer(event) {
  ...
  try {
    ...
    await fsMod.setDoc(fsMod.doc(state.db, 'gameSubmissions', submissionId), {
      ...
      submittedAt: nowISO(),
      elapsedMs: state.gameState.phase === 'live' && state.gameState.roundEndsAt ? Math.max(0, Number(problem.solveTimeMinutes || 3) * 60000 - Math.max(0, state.gameState.roundEndsAt - Date.now())) : 0,
      updatedAt: nowISO()
    }, { merge: true });
...
function bindTeamAndParticipantControls() {
  ...
  document.querySelectorAll('[data-team-select]').forEach((select) => {
    const lock = () => { state.teamAssignmentEditingId = select.dataset.teamSelect || ''; };
    const unlock = () => {
      setTimeout(() => {
        if (document.activeElement !== select) {
          state.teamAssignmentEditingId = '';
          renderGameAdmin();
        }
      }, 120);
    };
    select.addEventListener('focus', lock);
    select.addEventListener('mousedown', lock);
    select.addEventListener('click', lock);
    select.addEventListener('blur', unlock);
...
function currentTeamSubmission(teamId) {
  return state.submissions.find((s) => s.sessionId === state.gameState.sessionId && s.roundId === state.gameState.currentRoundId && s.teamId === teamId) || null;
}
...
function problemCard(problem, mode) {
  ...
  const solved = mode === 'game' && isProblemPlayedInCurrentActiveRound(problem.id);
  const story = problem.storyStage || '스토리 단계가 없습니다.';
  return `
    <article class="item-card clickable ${selected ? 'selected' : ''} ${solved ? 'solved' : ''}" data-problem-id="${escapeAttr(problem.id)}">
      <div class="card-head">
        ...
        <div class="chips">
          ...
          ${mode === 'game' && solved ? '<span class="status-badge rejected">완료</span>' : ''}
        </div>
      </div>
...
function buildProblemMarkup(problem, mode, showAnswer, options = {}) {
  const images = problem.images || [];
  const isPresenter = Boolean(options.presenter);
  const mediaExtra = isPresenter ? renderLiveSubmissionPanel() : '';
  return `
    <div class="problem-screen ${mode}${isPresenter ? ' stage-problem-screen' : ''}">
      <section class="problem-main">
        ...
        ${isPresenter ? '' : `<h3 class="problem-title">${escapeHtml(problem.title || '새 문제')}</h3>`}
...
function formatTextHtml(text) {
  const normalized = normalizeDisplayText(String(text ?? '').replace(/\\/g, '\\'));
...
function activeViewId() {
  return els.views.find((view) => view.classList.contains('active'))?.id || 'overview-view';
}

function currentRoutePayload(viewId = activeViewId()) {
  return {
    gbView: viewId,
    gbAuthorMode: viewId === 'author-view' ? state.authorMode : 'browser',
    gbSelectedProblemId: viewId === 'author-view' ? (state.selectedProblemId || '') : '',
    gbLoadedProblemId: viewId === 'author-view' ? (state.loadedProblemId || '') : '',
    gbGuard: true
  };
}
...
function routeKey(payload) {
  if (!payload) return '';
  return [payload.gbView || '', payload.gbAuthorMode || '', payload.gbSelectedProblemId || '', payload.gbLoadedProblemId || ''].join('::');
}

function applyRoutePayload(payload) {
  if (!payload) return 'overview-view';
  const target = payload.gbView || 'overview-view';
  if (target === 'author-view') {
    state.authorMode = payload.gbAuthorMode === 'detail' ? 'detail' : 'browser';
    state.selectedProblemId = payload.gbSelectedProblemId || '';
    state.loadedProblemId = payload.gbLoadedProblemId || '';
    if (state.authorMode === 'detail' && state.selectedProblemId) {
      const problem = state.problems.find((p) => p.id === state.selectedProblemId);
      if (problem) loadProblem(problem, false);
      else state.authorMode = 'browser';
    }
  } else {
    state.authorMode = 'browser';
  }
...
function setupBrowserHistory() {
  ...
  const initialPayload = currentRoutePayload(activeViewId());
  try {
    history.replaceState(initialPayload, document.title, window.location.href);
    history.pushState(initialPayload, document.title, window.location.href);
  } catch (error) {
    console.error(error);
  }
...
function onBrowserPopState(event) {
  const payload = event.state && event.state.gbGuard ? event.state : currentRoutePayload(activeViewId());
  const target = applyRoutePayload(payload);
  switchView(target, { fromHistory: true });
  setTimeout(() => {
    try {
      const currentPayload = currentRoutePayload(activeViewId());
      if (!history.state?.gbGuard) history.pushState(currentPayload, document.title, window.location.href);
    } catch (error) {
      console.error(error);
    }
  }, 0);
...
function pushCurrentRoute({ replace = false } = {}) {
  if (!state.historyReady) return;
  try {
    const payload = currentRoutePayload(activeViewId());
    if (replace) history.replaceState(payload, document.title, window.location.href);
    else if (routeKey(history.state) !== routeKey(payload)) history.pushState(payload, document.title, window.location.href);
  } catch (error) {
    console.error(error);
  }
}
...
function switchView(viewId, options = {}) {
  ...
  if (state.historyReady && !options.fromHistory) pushCurrentRoute({ replace: Boolean(options.replaceHistory) });
}
...
function computeRoundDeltaMap() {
  const deltaMap = new Map(state.teams.map((team) => [team.id, 0]));
  currentSessionSubmissions().filter((s) => s.roundId === state.gameState.currentRoundId).forEach((submission) => {
    deltaMap.set(submission.teamId, (deltaMap.get(submission.teamId) || 0) + Number(submission.scoreAwarded || 0));
  });
  return deltaMap;
}

function scoreboardRowsMarkup(fullscreen = false) {
  const totals = computeTeamTotals();
  const deltaMap = computeRoundDeltaMap();
  const deltaTimeMap = computeRoundTimeDeltaMap();
  const tiedScores = new Set();
  const scoreCounts = new Map();
  totals.forEach((item) => scoreCounts.set(item.total, (scoreCounts.get(item.total) || 0) + 1));
  scoreCounts.forEach((count, score) => { if (count > 1) tiedScores.add(score); });
  const previousTotals = totals.map((item) => ({ ...item, previous: item.total - (deltaMap.get(item.teamId) || 0), previousTime: item.timeMs - (deltaTimeMap.get(item.teamId) || 0) }));
  const previousRanks = previousTotals.slice().sort((a, b) => b.previous - a.previous || a.previousTime - b.previousTime || a.teamName.localeCompare(b.teamName, 'ko'));
  const previousRankMap = new Map(previousRanks.map((item, index) => [item.teamId, index + 1]));
  return totals.length ? totals.map((item, index) => {
    const delta = deltaMap.get(item.teamId) || 0;
    const previousRank = previousRankMap.get(item.teamId) || (index + 1);
    const move = previousRank - (index + 1);
    const crown = index === 0 ? '<img class="scoreboard-crown" src="./assets/crown.png" alt="1위" />' : '';
    const moveText = move > 0 ? `▲${move}` : move < 0 ? `▼${Math.abs(move)}` : '유지';
    const moveClass = move > 0 ? 'rank-up' : move < 0 ? 'rank-down' : 'rank-same';
    const timeHtml = tiedScores.has(item.total) ? `<small>동점 판정 ${escapeHtml(formatElapsedMs(item.timeMs))}</small>` : '';
    return `
      <div class="scoreboard-row${fullscreen ? ' scoreboard-row-large' : ''}" style="animation-delay:${index * 90}ms">
        <div class="scoreboard-rank">${crown}<strong>${index + 1}위</strong></div>
        <div class="scoreboard-team"><strong>${escapeHtml(item.teamName)}</strong><small class="${moveClass}">${escapeHtml(moveText)}</small></div>
        <div class="scoreboard-delta ${delta > 0 ? 'delta-plus' : 'delta-zero'}">${delta > 0 ? `+${delta}점` : '변동 없음'}${timeHtml}</div>
        <div class="scoreboard-total">${escapeHtml(String(item.total))}점</div>
      </div>`;
  }).join('') : '<div class="empty-box">표시할 팀 점수가 없습니다.</div>';
}

function renderScoreboardOverlay() {
  if (!els.scoreboardBody) return;
  els.scoreboardSubtitle.textContent = state.gameState.currentProblemId ? '현재 누적 점수와 이번 문제 변동입니다.' : '누적 점수 기준 순위표입니다.';
  els.scoreboardBody.innerHTML = scoreboardRowsMarkup(false);
}

function openScoreboardOverlay(fromJeopardy = false) {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  if (fromJeopardy && els.jeopardyOverlay && !els.jeopardyOverlay.classList.contains('hidden')) {
    state.jeopardyReturnMode = state.jeopardyMode === 'scoreboard' ? 'board' : state.jeopardyMode;
    state.jeopardyMode = 'scoreboard';
    renderJeopardyOverlay();
    return;
  }
  renderScoreboardOverlay();
  els.scoreboardOverlay?.classList.remove('hidden');
  state.scoreboardShownAt = Date.now();
}

function closeScoreboardOverlay() {
  els.scoreboardOverlay?.classList.add('hidden');
}

function renderAdminScoreboard() {
  if (!els.adminScoreboard) return;
  if (els.scoreboardOverlay && !els.scoreboardOverlay.classList.contains('hidden')) renderScoreboardOverlay();
  if (!isOperator()) {
...
function renderRoundResults() {
  ...
  const roundSubs = currentSessionSubmissions().filter((s) => s.roundId === state.gameState.currentRoundId).sort((a, b) => Number(b.scoreAwarded || 0) - Number(a.scoreAwarded || 0));
...
async function applyTeamAssignmentFromExcel() {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  const file = els.teamAssignFile?.files?.[0];
  if (!file) return toast('엑셀 파일을 선택해야 합니다.', 'error');
  if (!window.XLSX) return toast('엑셀 처리 라이브러리를 불러오지 못했습니다.', 'error');
  const { fsMod } = requireFirebaseReady();
  try {
    const wb = window.XLSX.read(await file.arrayBuffer(), { type: 'array' });
    const sheet = wb.Sheets[wb.SheetNames[0]];
    const rows = window.XLSX.utils.sheet_to_json(sheet, { defval: '' });
    if (!rows.length) return toast('엑셀 시트에 데이터가 없습니다.', 'error');
    const parsed = rows.map((row) => ({
      name: String(row['이름'] || row['name'] || row['Name'] || '').trim(),
      team: String(row['팀'] || row['team'] || row['Team'] || '').trim()
    })).filter((row) => row.name && row.team);
    if (!parsed.length) return toast('엑셀에는 이름 / 팀 열이 필요합니다.', 'error');

    const teamMap = new Map(state.teams.map((team) => [team.name, team]));
    let nextOrder = state.teams.length + 1;
    for (const row of parsed) {
      if (!teamMap.has(row.team)) {
        const ref = fsMod.doc(fsMod.collection(state.db, 'gameTeams'));
        const team = { id: ref.id, name: row.team, order: nextOrder++, createdAt: nowISO(), updatedAt: nowISO() };
        await fsMod.setDoc(ref, team);
        teamMap.set(row.team, team);
      }
    }

    const participantMap = new Map(visibleParticipants().map((participant) => [normalizeRosterName(participant.displayName), participant]));
    let assigned = 0;
    for (const row of parsed) {
      const participant = participantMap.get(normalizeRosterName(row.name));
      const team = teamMap.get(row.team);
      if (!participant || !team) continue;
      await fsMod.setDoc(fsMod.doc(state.db, 'gameParticipants', participant.id), { teamId: team.id, updatedAt: nowISO(), lastSeenAt: Date.now() }, { merge: true });
      assigned += 1;
    }
    toast(`${assigned}명의 참가자를 엑셀 기준으로 자동 배정했습니다.`, 'success');
    if (els.teamAssignFile) els.teamAssignFile.value = '';
  } catch (error) {
    console.error(error);
    toast(`엑셀 자동 배정에 실패했습니다: ${error.message}`, 'error');
  }
}

function exportResultsWorkbook() {
  if (!isOperator()) return toast('운영자 권한이 필요합니다.', 'error');
  if (!window.XLSX) return toast('엑셀 처리 라이브러리를 불러오지 못했습니다.', 'error');
  const totals = computeTeamTotals();
  const participantsForExport = currentGameParticipants();
  const participantTeamIds = new Set(participantsForExport.map((participant) => participant.teamId).filter(Boolean));
  const submittedTeamIds = new Set(currentSessionSubmissions().map((submission) => submission.teamId).filter(Boolean));
  const exportTeamIds = new Set([...participantTeamIds, ...submittedTeamIds, ...totals.filter((item) => item.total || item.timeMs).map((item) => item.teamId)]);
  const exportTeams = state.teams.filter((team) => exportTeamIds.has(team.id));
  const memberRows = exportTeams.flatMap((team) => {
    const members = participantsForExport.filter((participant) => participant.teamId === team.id);
    return members.length ? members.map((member, idx) => ({ 순위: '', 팀명: team.name, 팀점수: '', 팀원순번: idx + 1, 이름: member.displayName || '', 이메일: member.email || '' })) : [{ 순위: '', 팀명: team.name, 팀점수: '', 팀원순번: '', 이름: '', 이메일: '' }];
  });
  const teamRows = totals.filter((item) => exportTeamIds.has(item.teamId)).map((item, index) => {
    const members = participantsForExport.filter((participant) => participant.teamId === item.teamId).map((participant) => participant.displayName || '').join(', ');
    return { 순위: index + 1, 팀명: item.teamName, 점수: item.total, 총제출시간: formatElapsedMs(item.timeMs), 팀원수: participantsForExport.filter((participant) => participant.teamId === item.teamId).length, 팀원목록: members };
  });
  const wb = window.XLSX.utils.book_new();
  const ws1 = window.XLSX.utils.json_to_sheet(teamRows.length ? teamRows : [{ 순위: '', 팀명: '', 점수: '', 총제출시간: '', 팀원수: '', 팀원목록: '' }]);
  const ws2 = window.XLSX.utils.json_to_sheet(memberRows.length ? memberRows : [{ 순위: '', 팀명: '', 팀점수: '', 팀원순번: '', 이름: '', 이메일: '' }]);
  window.XLSX.utils.book_append_sheet(wb, ws1, '팀 순위');
  window.XLSX.utils.book_append_sheet(wb, ws2, '팀원 목록');
  window.XLSX.writeFile(wb, `goldenbell-results-${Date.now()}.xlsx`);
  toast('현재 게임 결과 엑셀 파일을 저장했습니다.', 'success');
}
...
async function setGameState(patch) {
  ...
  refreshAll();
  await fsMod.setDoc(fsMod.doc(state.db, 'gameState', CONFIG.gameStateDocId), data, { merge: true });
}
